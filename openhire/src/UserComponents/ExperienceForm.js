...
export default class ExperienceForm extends Component {
	...
	componentDidUpdate = (prevProps) => {
		if (this.props.pageAddress !== prevProps.pageAddress) {
			this.setState({ pageAddress: this.props.pageAddress, ...defaultForm }, () => {
				this.updatePage();
			});
		}
	};
				...
				<h2>
					...
					{ownPage ? (
						<Icon
							link
							name="plus"
							onClick={() => this.setState({ displayExperienceForm: !displayExperienceForm })}
						/>
					) : (
						''
					)}
				...
				{displayExperienceForm ? (
					<div>
						...
						<Form onSubmit={this.addExperience}>
							...
							<Checkbox toggle checked={employer} onClick={() => this.toggleForm()} />
							<Label as="a" onClick={() => this.toggleForm(true)} color={employer ? 'blue' : 'grey'}>
								<Icon name="building" />
								Employer
							</Label>
							<Form.Input
